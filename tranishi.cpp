...
void tra_nishi::constructor(const char *codname,const char *pdbname, int n)
{
	...
	while(!ifs.eof()){// to read binary
		...
		double minx=99999,maxx=-99999,miny=99999,maxy=-99999,minz=99999,maxz=-99999;
                for(unsigned int w=0; w < pdb1->total_atom; w++){
                        ifs.read( (char *) &buffloat, 4 );
			//if(minx>buffloat&&pdb1->resn[w]!="WAT")minx=buffloat;
			//if(maxx<buffloat&&pdb1->resn[w]!="WAT")maxx=buffloat;
			if(minx>buffloat&&pdb1->resn[w]!="WAT")minx=buffloat;
			if(maxx<buffloat&&pdb1->resn[w]!="WAT")maxx=buffloat;
                        cordx.push_back(buffloat);
                        ifs.read( (char *) &buffloat, 4 );
			if(miny>buffloat)miny=buffloat;
			if(maxy<buffloat)maxy=buffloat;
                        cordy.push_back(buffloat);
                        ifs.read( (char *) &buffloat, 4 );
			if(minz>buffloat)minz=buffloat;
			if(maxz<buffloat)maxz=buffloat;
                        cordz.push_back(buffloat);
...
int tra_nishi::write_step(const char* filename,int n){//output pdb at n step
   ...
   if( (unsigned) n >= total_step ){ //added 2015.1.30
	...
	char bfch;
		...
		bfrnum=pdb1->rnum[j];
                bfcoox=cordx[n * pdb1->total_atom + j];
                bfcooy=cordy[n * pdb1->total_atom + j];
                bfcooz=cordz[n * pdb1->total_atom + j];
                bfoccu=pdb1->occu[j];
                bftemf=pdb1->temf[j];
...
int tra_nishi::fix_cod(float fxcell,float fycell,float fzcell){//replicate atoms because of periodic boundary condition
	...
	int imove;
        ...
        for(unsigned int j = n * pdb1->total_atom +1; j < (n+1) * pdb1->total_atom;j++){
                ...
                //cout<<"cordx before = "<<cordx[j]<<endl;
                cordx[j] = cordx[j] - imove*fxcell;
                //cout<<"cordx after = "<<cordx[j]<<endl;
                //if(rdiff<0&&rdiff>fxcell/2)cordx[j] = cordx[j] + fxcell;
                rdiff = cordy[j]-cordy[j-1];
                //rdiff = cordy[j]-cordy[j-pdb1->total_atom];
                if(rdiff>=0) imove = int(rdiff/fycell+0.5);
                else imove = int(rdiff/fycell-0.5);
                cordy[j] = cordy[j] - imove*fycell;
                //if(rdiff<0&&rdiff>fycell/2)cordy[j] = cordy[j] + fycell;
                rdiff = cordz[j]-cordz[j-1];
                //rdiff = cordz[j]-cordz[j-pdb1->total_atom];
                if(rdiff>=0) imove = int(rdiff/fzcell+0.5);
                else imove = int(rdiff/fzcell-0.5);
                cordz[j] = cordz[j] - imove*fzcell;
                //if(rdiff<0&&rdiff>fzcell/2)cordz[j] = cordz[j] + fzcell;
                //cout<<j<<" ";
        }
        }
